<?php
/*
 * Template Name: Stripe Event Template DOP
...
<event ng-app="wcEvent" ng-controller="eventController" ng-class="{
                    ...
                    <event-card>
                        ...
                        <card--right class="card-right--stripe event-card--padding">
                            <card-container ng-cloak>
                                ...
                                <br>
                                <card-stripe class="card-stripe" id="stripe" ng-hide="fields.emails.length !== fields.eventQty">
    ...
    <event--payment-voucher class="ng-hide" ng-hide="!fields.showPaymentVoucher">
        <payment--voucher-container>
            ...
            <voucher--header><?= __("Stripe order", 'sp') ?></voucher--header>
...
<script type="text/javascript">
    ...
    app.constant("EVENT_CONF", {
            ...
            updatePaymentIntent: home_url("/wp-json/wctvApi/v1/stripe/updatePaymentIntent"),
            retrivePaymentIntentInfos: home_url("/wp-json/wctvApi/v1/stripe/retrivePaymentIntentInfos"),
        })
        ...
        .controller("eventController", [
            ...
            function($scope, $eventResource, EVENT_CONF, $window, $sce, $interval) {
                    ...
                    if (paymentId) {
                        ...
                        $eventResource.retrivePaymentIntentInfos({
                            ...
                            if (paymentIntentResponse.data.status === "succeeded") {
                                ...
                                stripePaymentSuccess(paymentIntentResponse.data);
                            } else {
                                const error = new Error(`Stripe payment fail with status: ${paymentIntentResponse.data.status}`);
                                striPaymentCaptureError(error);
                                showLoadingVeil();
                            }
                        }, (err) => {
                            showLoadingVeil();
                            ...
                            __atatusNotify(new Error("stripePaymentError: " + [invoiceID, err].join("-")));
                        });
                    }
                }

                function stripePaymentSuccess(details) {
                    try {
                        debugger
                        ...
                        $scope.fields = metadata; // JSON.parse(atob(fields));
                        const charge = data[0];
                        const {
                            billing_details
                        } = charge;
                        ...
                        let params = {
                            ...
                            eventPrice: metadata.eventPrice
                ...
                function striPaymentCaptureError(error) {
                    hideLoadingVeil();
                    __atatusNotify(error);

                    alert("<?= __("STRIPE ERROR: no fue posible procesar su pago debido a que stripe retornó un error, intente nuevamente", 'sp') ?>");
                }
                ...
                function updatePaymentIntent() {
                    if (PAYMENT_ID) {
                        ...
                        Object.keys(fields).forEach(prop => {
                            metadata.push(`metadata[${prop}]=${encodeURIComponent(fields[prop])}`);
                        });
                        ...
                        const updatePayment = {
                            ...
                            currency: 'dop', // mat to stripe currency
                            // customer : "<?= the_ID() ?>",
                        ...
                        $eventResource.updatePaymentIntent(updatePayment, (updatePaymentResponse) => {
                            setLoading(false);
                            console.log("updatePayment: ", updatePayment);
                            console.log("updatePaymentResponse: ", updatePaymentResponse);
                        }, (error) => {

                            alert("<?= __("Unable to update the payment information please reload your browser.", 'sp') ?>")
                            setLoading(false);
                        });
                    }
...
<script type="text/javascript">
    ...
    const stripe = Stripe("pk_test_51OK0B3HKqIDcAZwyRPTPnoreHBFl8DCmVpBpxokVEgX9Zy9uj6T1VW4tU5CpuxuJoqpCBauo7sZHlZultAHP99Od00s78fTjqM");
    // const stripe = Stripe("pk_live_51L6Po1DlmL2vFzfy21cjJGKvo2oVhv7AtkfuNHnJkXPSNpyegYfP0bu43dQIbVL5Qj2w3cmlJXgmy5dzNC1WtGHB00RTjV6ud5");
    ...
    async function initialize() {
        ...
        const response = await fetch(home_url("/wp-json/wctvApi/v1/stripe/createPaymentID"), {
            ...
            body: JSON.stringify({
                amount: Number(<?= sprintf('%0.2f', get_field("event_price")) ?> || 0),
                eventId: "<?= the_ID() ?>",
                statementDescriptor: "<?= the_title() ?>",
                invoiceId: invoiceID,
            }),
    ...
    async function handleSubmit(e) {
        ...
        if (error.type === "card_error" || error.type === "validation_error") {
            ...
            showMessage("<?= __("An unexpected error occurred.", 'sp') ?>");
        }
    ...
    async function checkStatus() {
        ...
        switch (paymentIntent.status) {
            case "succeeded":
                ...
                showMessage("<?= __("Payment succeeded!", 'sp') ?>");
                break;
            case "processing":
                showMessage("<?= __("Your payment is processing.", 'sp') ?>");
                break;
            case "requires_payment_method":
                showMessage("<?= __("Your payment was not successful, please try again.", 'sp') ?>");
                break;
            default:
                showMessage("<?= __("Something went wrong.", 'sp') ?>");
                break;
        }
    }
